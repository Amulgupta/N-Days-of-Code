...
int main() {
    ...
    a += '+' + b;
    SuffixArray sa(a);
    int id = INT_MAX, ans = 0, cur = 0;
    bool foi = false;
    for(int i = 0; i < (int)sa.sa.size() - 1; i++) {
        if(sa.sa[i] < n and sa.sa[i + 1] > n) {
            foi = true;
            cur = sa.lcp_v[i];
            if(cur > ans) {
                ans = cur;
        ...
        } else if(foi and sa.sa[i] > n and sa.sa[i + 1] > n) {
            cur = min(cur, sa.lcp_v[i]);
            if(cur == ans) {
                id = min(id, sa.sa[i + 1]);
            }
        } else {
    ...
    foi = false;
    for(int i = (int)sa.sa.size() - 2; i >= 0; i--) {
        if(sa.sa[i] > n and sa.sa[i + 1] < n) {
            foi = true;
            cur = sa.lcp_v[i];
            if(cur > ans) {
